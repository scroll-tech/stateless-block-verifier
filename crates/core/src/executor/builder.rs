...
impl<'e> EvmExecutorBuilder<'e, ()> {
    ...
    pub fn new(zk_db: Rc<ZkMemoryDb>) -> Self {
        Self {
            hardfork_config: (),
...
impl<'e, H> EvmExecutorBuilder<'e, H> {
    ...
    /// Set zktrie state.
    pub fn zktrie_state(self, zk_db: Rc<ZkMemoryDb>) -> EvmExecutorBuilder<'e, H> {
        EvmExecutorBuilder { zk_db, ..self }
    }
}
...
impl<'e> EvmExecutorBuilder<'e, HardforkConfig> {
    ...
    pub fn build<T: Block>(self, l2_trace: &T) -> Result<EvmExecutor<'e>, ZkTrieError> {
        ...
        let db = cycle_track!(
            CacheDB::new(ReadOnlyDB::new(l2_trace, &self.zk_db)?),
            "build ReadOnlyDB"
        );
